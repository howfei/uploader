...
public class Webs {
	...
	public static final String ROOT_DOMAIN_PATTERN 	= ".*\\.(?=.*\\.)";
	...
	public static String getRootDomain(HttpServletRequest request){
		String rootDomain = request.getServerName().replaceAll(ROOT_DOMAIN_PATTERN, "");
		return rootDomain;
	}
	 ...
	 * @return
	 */
	public static String getIfNoneMatch(HttpServletRequest request){
		return request.getHeader(HEADER_IF_NONE_MATCH);//与服务端ETag值对应//"If-None-Match"
	}
	 ...
	 * Date
	 */
	public static Date getIfModifiedSince(HttpServletRequest request){
		long header_if_modified_since = request.getDateHeader(HEADER_IF_MODIFIED_SINCE);
		return header_if_modified_since > -1 ? new Date(header_if_modified_since) : null;
	}
	
	public static void setLastModified(HttpServletResponse response, Date updated, String eTag){
		SimpleDateFormat formater = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss zzz", Locale.ENGLISH);
		response.addHeader("Last-Modified", formater.format(updated));
