...
import org.howsun.wrs.util.Webs;
...
public class Page implements Serializable {
	...
	public Page(HttpServletRequest request, int pageSize, String url){
		this(Webs.getIntByRequestParameter(request, PAGE_NUMBER_PARAMETER_NAME, 1), pageSize,
			 Webs.getIntByRequestParameter(request, PAGE_TOTAL_PARAMETER_NAME, 0));
		if(Strings.hasLength( url) ){
			String param = Webs.param(request, Page.PAGE_NUMBER_PARAMETER_NAME, Page.PAGE_TOTAL_PARAMETER_NAME);
			if(Strings.hasLength(param)){
				url+="?" + param;
			}
			this.url = url;
		}
	 ...
	 * void
	 */
	public void urlEncodingWhenNonAscii(){
		if(this.url != null){
			StringBuilder sb = new StringBuilder();
			char[] chars = this.url.toCharArray();
			for(char c : chars){
				boolean isChinese = Strings.isChinese(c);
				if(isChinese){
					sb.append(Codings.urlEncode(Character.toString(c)));
				}else{
					sb.append(c);
				}
			}
